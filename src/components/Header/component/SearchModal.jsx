...
const SearchModal = ({open, onCancel}) => {
  ...
  return (
    <Modal open={open}>
      <CardContainer className='p-2 w-1/3'>
        <div className='w-full mb-6 flex justify-between'>
          <Heading text={'Search'}/>
          <IconButton onClick={onCancel} icon={
          ...
          <div className="w-full flex flex-col">
            ...
            <div className="w-full mb-6 flex flex-col justify-center">
              ...
              <select
                id="model"
                className="select select-md select-primary select-bordered w-full text-black bg-neutral-50"
                disabled={!filter?.make || filter?.make === 'any' }
              >
                <option value={'any'}>Any</option>
                {filterOptions?.make
                  ?.find((make) => make.name === filter?.make) // Find selected make
                  ?.models.map((model, i) => (
        ...
        <div className='mb-4 mt-8 w-full flex items-center'>
          <a href={`${route.SHOPPING}${convertToQueryString(filter) ? `?${convertToQueryString(filter)}` : ''}`} className='btn w-32 btn-primary text-white mx-auto'>Search</a>
        </div>
      </CardContainer>
    </Modal>
  )
