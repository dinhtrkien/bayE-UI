    ...
    <CardContainer className={`py-8 w-1/4 ${className}`}>
      <div className="w-full">
        <div className="w-full mb-6 flex flex-col justify-center">
          ...
          <select
            id="location"
            className="select select-md select-primary select-bordered w-full  max-w-xs text-black bg-neutral-50"
          >
            {filterOptions?.locations.map((location) =>
              location.toUpperCase() === 'HA NOI' ? (
        ...
        <div className="w-full mb-6 flex flex-col justify-center">
          ...
          <select
            ...
            className="select select-md select-primary select-bordered w-full max-w-xs text-black bg-neutral-50"
          >
            {filterOptions?.make.map((condition) => (
              <option value={condition}>{condition}</option>
            ))}
          </select>
        </div>
